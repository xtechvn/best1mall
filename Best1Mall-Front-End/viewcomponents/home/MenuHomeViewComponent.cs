...
{
    public class MenuHomeViewComponent: ViewComponent
    {
        private readonly IConfiguration configuration;
        private readonly RedisConn _redisService;
        private readonly IMemoryCache _cache; // Inject IMemoryCache
        public MenuHomeViewComponent(IConfiguration _Configuration, RedisConn redisService, IMemoryCache cache)
        {
        ...
        public async Task<IViewComponentResult?> InvokeAsync()
        {
            try
            {
                ...
                return View("~/Views/Shared/Components/Home/MenuHome.cshtml", cached_view);
            }
            catch (Exception)
            {
                return Content("");
